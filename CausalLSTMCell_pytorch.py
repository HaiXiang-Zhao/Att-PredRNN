...
class CausalLSTMCell(nn.Module):
    def __init__(self, layer_name,num_hidden_in,num_hidden_out,
        ...
        self.h_cc_conv = nn.Conv2d(self.num_hidden_out,self.num_hidden_out*4,5,1,2).to(device)
        self.c_cc_conv = nn.Conv2d(self.num_hidden_out,self.num_hidden_out*3,5,1,2).to(device)
        self.m_cc_conv = nn.Conv2d(self.num_hidden_out,self.num_hidden_out*3,5,1,2).to(device)
        self.x_cc_conv = nn.Conv2d(self.num_hidden_in,self.num_hidden_out*7,5,1,2).to(device)
        self.c2m_conv  = nn.Conv2d(self.num_hidden_out,self.num_hidden_out*4,5,1,2).to(device)
        self.o_m_conv = nn.Conv2d(self.num_hidden_out,self.num_hidden_out,5,1,2).to(device)
        self.o_conv = nn.Conv2d(self.num_hidden_out, self.num_hidden_out, 5, 1, 2).to(device)
        self.cell_conv = nn.Conv2d(self.num_hidden_out*2,self.num_hidden_out,1,1,0).to(device)


    def init_state(self):
        return torch.zeros((self.batch, self.num_hidden_out,self.width,self.height),dtype=torch.float32).to(device)
